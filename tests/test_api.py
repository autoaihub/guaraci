...
def test_source_schema_endpoint_zikavirus_contains_expected_fields(client: TestClient) -> None:
    response = client.get("/sources/zikavirus/schema")

    assert response.status_code == 200
    payload = response.json()
    names = {item["name"] for item in payload["params"]}
    assert payload["source"] == "zikavirus"
    assert payload["mode"] == "opendatasus api"
